...
// Helper to generate random traffic pattern
const useTrafficPattern = (seed: string) => {
  return useMemo(() => {
    // Generate 3 distinct particles with different speeds/delays
    return [1, 2, 3].map((i) => ({
      id: i,
      speed: 1.5 + Math.random() * 2, // Random speed between 1.5s and 3.5s
      delay: Math.random() * -2,      // Negative delay to start mid-path
      size: 3 + Math.random() * 2,    // Random size (slightly larger)
      colorOffset: Math.random() > 0.5 ? '#fff' : 'currentColor'
    }));
  }, [seed]);
};
    ...
    <>
      <BaseEdge 
        ...
        style={{
            ...
            // If animated, we want a solid, dim line (not dashed) to act as the "rail" for particles
            // We override dasharray to 'none' to kill the default CSS animation
            strokeDasharray: (animated && !isSketch) ? 'none' : style.strokeDasharray,
            strokeOpacity: railOpacity, 
            animation: 'none', // Kill CSS animation
            filter: edgeFilter
        }} 
      />

      {/* --- TRAFFIC ANIMATION (Particles) - Disable in Sketch Mode --- */}
      {animated && !isSketch && !isRemoved && particles.map((p) => (
          <circle 
            key={p.id} 
            r={p.size} 
            fill={p.colorOffset === '#fff' ? '#ffffff' : strokeColor}
            filter="drop-shadow(0 0 3px currentColor)" // Add glow to particles
          >
            <animateMotion 
                dur={`${p.speed}s`} 
